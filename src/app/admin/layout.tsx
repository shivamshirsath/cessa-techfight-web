...
import { LayoutDashboard, Calendar, Users, IndianRupee, LogOut, Loader2, ShieldAlert } from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [userRole, setUserRole] = useState<'ADMIN' | 'TREASURER' | null>(null);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        // 1. Fetch the exact role of the logged-in user
        const userDoc = await getDoc(doc(db, 'users', user.uid));
        const role = userDoc.data()?.role;

        // 2. Security Check: Are they allowed here at all?
        if (role !== 'ADMIN' && role !== 'TREASURER') {
          alert("Unauthorized access. Only Admins and Treasurers can view this area.");
          router.push('/');
          return;
        }

        // 3. THE BOUNCER: If Treasurer tries to access an Admin page, kick them back to finance!
  ...
  // Prevent rendering anything if the role wasn't set properly
      ...
      <aside className="w-64 bg-white border-r border-gray-200 hidden md:flex flex-col">
        <div className="h-16 flex items-center px-6 border-b border-gray-100 shrink-0">
          <span className="text-xl font-extrabold text-gray-900 tracking-tight flex items-center gap-2">
            <ShieldAlert className={userRole === 'ADMIN' ? 'text-purple-600' : 'text-green-600'} size={24}/>
            {userRole === 'ADMIN' ? 'Admin Portal' : 'Finance Portal'}
          </span>
        </div>
